...
In Part 1, we identified the root cause of our instability: a "Fan-Out" architecture. A single user request triggered 20 internal API calls. This turned our own traffic into a weapon against our backend.
...
In most browsers and standard Node.js configurations, HTTP requests have a default timeout of around **300 seconds (5 minutes)**. However, Undici does not set a default timeout. It will wait indefinitely.
...
This massive burst of CPU activity locks the Event Loop completely. While the loop is busy clearing this backlog, Kubernetes pings the `/healthz` endpoint. Node.js is too busy to answer. Kubernetes marks the pod as unhealthy and kills it.
...
We stopped being polite to our dependencies. We wrapped our external calls in circuit breakers and enforced strict timeouts. If a downstream service hangs, we cut the connection early. This prevents the Event Loop queue from becoming unmanageable.
